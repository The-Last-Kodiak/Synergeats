...
router.post("/login", (req, res) => {
  ...
  if (typeof username !== "string" || typeof password !== "string") {
    ...
    import_credential_svc.default.verify(username, password).then((goodUser) => generateAccessToken(goodUser)).then((token) => res.status(200).send({ token })).catch((_error) => res.status(401).send("Unauthorized"));
  }
});
