...
const credentialSchema = new Schema<Credential>(
  ...
  {
    // collection name from the lab instructions
...
// Create a new credential: hash the password, save username + hash.
// IMPORTANT: does NOT store the clear-text password.
function create(
  username: string,
  password: string
): Promise<Credential> {
...
// Returns the username if successful, or rejects if not.
function verify(
  username: string,
  password: string
): Promise<string> {
